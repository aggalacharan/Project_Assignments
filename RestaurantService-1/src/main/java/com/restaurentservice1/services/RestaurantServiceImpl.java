...
import com.restaurentservice.exceptions.ResourceNotFoundException;
...
//		Restaurant opt = restrepo.findById(restaurant.getRestaurantId().orElseThrow(
//				() -> new ResourceNotFoundException("Restaurant", "id", restaurant)
//				);
////		if(opt.isPresent()) {
////			throw new RestaurantException("Restaurant already exists..");
////		}else {
////			restaurant.getItemList().forEach(item -> item.setRestaurant(restaurant));
//	        
//			return restrepo.save(opt);
//		
//	}
	
	@Override
	public Restaurant addRestaurant(Restaurant restaurant) {
	    Restaurant opt = restrepo.findById(restaurant.getRestaurantId())
	            .orElseThrow(() -> new ResourceNotFoundException("Restaurant", "id", restaurant.getRestaurantId()));
